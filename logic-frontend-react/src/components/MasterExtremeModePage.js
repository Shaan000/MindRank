...
export default function MasterExtremeModePage({ user, accessToken, authInitialized }) {
  ...
  useEffect(() => {
    ...
    const skipInstructions = localStorage.getItem('skipMasterExtremeInstructions') === 'true';
    if (!skipInstructions) {
  ...
  // Enhanced format statements function for complex extreme logic modes
  const formatComplexStatements = (statementData) => {
    ...
    return Object.entries(statementData).map(([speaker, data]) => {
      ...
      if (data.mode === 'XOR') {
        const c1Text = data.c1 ? 'Truth-Teller' : 'Liar';
        const c2Text = data.c2 ? 'Truth-Teller' : 'Liar';
        return `${speaker} says: "EITHER ${data.t1} is a ${c1Text} OR ${data.t2} is a ${c2Text}, but NOT both."`;
      }
      
      if (data.mode === 'IFF') {
        const c1Text = data.c1 ? 'Truth-Teller' : 'Liar';
        const c2Text = data.c2 ? 'Truth-Teller' : 'Liar';
        return `${speaker} says: "${data.t1} is a ${c1Text} IF AND ONLY IF ${data.t2} is a ${c2Text}."`;
      }
      
      if (data.mode === 'NESTED_IF') {
        const outerCondText = data.outer_val ? 'Truth-Teller' : 'Liar';
        const innerCondText = data.inner_val ? 'Truth-Teller' : 'Liar';
        const innerResultText = data.inner_result_val ? 'Truth-Teller' : 'Liar';
        return `${speaker} says: "IF ${data.outer_cond} is a ${outerCondText}, THEN IF ${data.inner_cond} is a ${innerCondText}, THEN ${data.inner_result} is a ${innerResultText}."`;
      }
      ...
      if (data.mode === 'GROUP') {
        ...
        let memberText;
        if (members.length === 2) {
          memberText = `${members[0]} and ${members[1]}`;
        } else if (members.length === 3) {
          memberText = `${members[0]}, ${members[1]}, and ${members[2]}`;
        } else {
          memberText = members.slice(0, -1).join(', ') + `, and ${members[members.length - 1]}`;
        }
        return `${speaker} says: "Exactly ${data.exactly} of ${memberText} are Truth-Tellers."`;
      }
  ...
  const getPlayerList = () => {
    ...
    if (puzzle.full_statement_data && typeof puzzle.full_statement_data === 'object') {
      ...
      Object.entries(puzzle.full_statement_data).forEach(([speaker, data]) => {
        ...
        if (data.outer_cond) players.add(data.outer_cond);
        if (data.inner_cond) players.add(data.inner_cond);
        if (data.inner_result) players.add(data.inner_result);
        if (data.members) data.members.forEach(member => players.add(member));
      });
  ...
  return (
    <div style={appStyle}>
      ...
      <div style={headerStyle}>
        ...
        <h1 style={titleStyle}>Master Extreme Mode 👑</h1>
        <p style={{
          fontSize: '1.25rem',
      ...
      <div style={sectionStyle}>
        ...
        {isLoading ? (
          <div style={{...cardStyle, textAlign: 'center'}}>
            <p style={{color: '#b0a99f', fontSize: '1.25rem'}}>Generating master extreme puzzle...</p>
          </div>
        ) : puzzle ? (
          <div style={cardStyle}>
            <h3 style={{
            ...
            {result === 'correct' && (
              <div style={{...cardStyle, background: '#1e4a2a', border: '1px solid #28a745', marginTop: '2rem', textAlign: 'center'}}>
                <p style={{margin: 0, color: '#ffffff', fontSize: '1.25rem'}}>👑 LEGENDARY! True logic mastery achieved!</p>
              </div>
            )}
      ...
      {showInstructions && (
        <InstructionsModal
          ...
          skipKey="skipMasterExtremeInstructions"
